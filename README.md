...
本插件从原 [(https://github.com/MCYUNIDC/mccloud_img)](https://github.com/MCYUNIDC/mccloud_img) 分支派生，经历了彻底的底层重构，从单一的图片获取工具，进化为**支持多指令分流、精细化分群管理、阅后即焚与安全防护**的高级图片引擎。
...
相较于早期版本，本次重构在安全、管控与交互上实现了质的飞跃：
*   **从“无差别发送”到“精细化控群”**：全面引入下拉式**黑/白名单机制**，每一个图源都可以独立设置响应群组，轻松隔离 NSFW 指令。
*   **从“发送不管”到“阅后即焚”**：深度适配底层协议，实现**双重自动撤回**功能。图片与倒计时提示文本将在设定时间后同步销毁，不留痕迹。
*   **从“冰冷提示”到“猫娘模式”**：新增趣味互动开关，所有提示文本末尾自动追加自定义后缀词（如“喵~”）。
*   **安全与容错防护**：加入 **防 SSRF 攻击**（屏蔽内网与敏感 IP 抓取）、**防 OOM 内存溢出**（分块流式限制下载体积）以及完善的日志追踪系统。
*   **多源分流与直链兜底**：保留了强悍的多图源配置引擎、图片智能压缩与超时重试机制，发送失败必定给原图直链兜底。
...
*   **👻 自动撤回机制**：向撤回失败说再见。绕过封装直达底层协议，发送与撤回如丝般顺滑。
*   **⚡ 自动压缩防超时**：内置 `Pillow` 引擎，自定义“压缩阈值”，突破 QQ 协议端大图发不出的瓶颈。
*   **⏱️ 冷却防刷屏**：内置用户级冷却计时器，保护你的 API 与机器人不被恶意刷爆。
...
* 点击 **“添加 图源配置”**。
* **规则名称**：随意命名，如“涩图”。
* **触发指令**：输入你想要的触发词，多个请分行或使用列表（如 `涩涩`, `来点图`）。
...
* **纯文本 fallback**：当你看到机器人回复 `⚠️ 发送失败(重试X次)，原图直链：...` 时，说明底层 QQ 协议端在规定的时间内未能将图片上传至服务器，直接点击链接即可查看原图。
...
*   **v5.0 - v5.2** 
    *   🛡️ 安全性史诗级强化：新增防 SSRF 注入、防 OOM 内存溢出、修复内存泄露隐患。
    *   ✨ 新增下拉式黑/白名单群组管控功能。
    *   ✨ 首次引入自动撤回（阅后即焚）功能。
*   **v4.8 - v4.9** 
    *   🔁 新增发送失败自动重试机制。
    *   🌐 新增终极兜底方案：多次发送失败后自动转为发送原图直链文本。
*   **v4.0 - v4.7** 
